 ...
 * Supports: Both standard tabular grids (.crud-table) and card-based wallets (.ticket-card).
 */
document.addEventListener('DOMContentLoaded', function () {
    // Detect data source (Table or Card List)
    const table = document.querySelector('.crud-table');
    const cardContainer = document.querySelector('.ticket-container');
    
    if (!table && !cardContainer) return;

    // --- [1] SEARCH HUB CONSTRUCTION ---
    ...
    if (table) {
        ...
        // Hardcoded categories for the Ticket Wallet based on typical fields
        categories = [
    ...
    function populateOptions() {
        ...
        const cleanValue = (val) => {
            if (!val) return "";
            // Extract primary city (strip " to ...", " → ...", or ", Country")
        ...
        // Add both the cleaned city AND the full string (to be safe)
